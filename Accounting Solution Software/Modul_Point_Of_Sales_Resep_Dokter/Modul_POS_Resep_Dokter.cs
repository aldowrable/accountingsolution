...
namespace Accounting_Solution_Software
{
    public partial class Modul_POS_Resep_Dokter : Form
    {
        ...
        public Modul_POS_Resep_Dokter(Login login)
        {
            InitializeComponent();
            this.KeyPreview = true;
            allfunc = new AllFunc();
        ...
        public void setModulObatBebas(Modul_POS posModul)
        {
            this.posModul = posModul;
        }
        ...
        public void setCustomer(string[] custDataArr)
        {
            ...
            alamatPelangganTxt.Text = custDataArr[1];
        }
        ...
        {
            this.custDataArr2[0] = namaPasienTxt.Text;
            this.custDataArr2[1] = umurPasienTxt.Text;
            this.custDataArr2[2] = alamatPasienTxt.Text;
            return this.custDataArr2;
        }

        public string[] getDokter()
        {
            string[] arrDokter = new string[] { kodeDokter1Txt.Text, namaDokter1Txt.Text,noResepTxt.Text };
            return arrDokter;
        }
        ...
        private void openModulPembayaran()
        {
            ...
            if ((kodeDokter1Txt.Text.CompareTo("") != 0) && (noResepTxt.Text.CompareTo("") != 0) && (itemPenjualanDataGridView.Rows.Count != 0))
            {
                if (cariTuslah())
                {
                    String NoResep = null;
                    SqlDataReader sdr = dbo.get_single_data(new String[] { "NoResep" }, "YEAR(TanggalJam) = YEAR(getdate())  AND NoResep", noResepTxt.Text, "NoBon");
                    while (sdr.Read()) { NoResep = (String)sdr["NoResep"]; }
                    if (NoResep==null)
                    {
                        if (pembayaran != null)
                        {
                    ...
                    {
                        MessageBox.Show("Duplikasi No Resep, silakan input No resep lain.");
                    }

                }
                else
                {
                    MessageBox.Show("Tuslah belum diinput.");
                }
            }
            else
            {
                MessageBox.Show("Silakan mengisi No Resep ,Kode Dokter dan Transaksi.");
            }
        }

        private void Modul_POS_Resep_Dokter_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyData)
            {
                case Keys.F3:
        ...
        private void hapusProdukBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Jasa Dokter sudah diinput");
            }
            
        }

        private bool cariJasaDokter()
        {
            bool jasaDokterExsist = false;
            for (int i = 0; i < itemPenjualanDataGridView.Rows.Count; i++)
            {
                if (itemPenjualanDataGridView.Rows[i].Cells[1].Value.ToString().ToLower().Contains("jasa"))
                {
                    jasaDokterExsist = true;
                }
            }
            return jasaDokterExsist;

        }

        private bool cariTuslah()
        {
            bool found = false;
            for (int i = 0; i < itemPenjualanDataGridView.Rows.Count; i++)
            {
                if (itemPenjualanDataGridView.Rows[i].Cells[1].Value.ToString().ToLower().Contains("tuslah"))
                {
                    found = true;
                }
            }
        ...
        public void setDataDokter(string[] arrDataDokter)
        {
            kodeDokter1Txt.Text = arrDataDokter[0];
            namaDokter1Txt.Text = arrDataDokter[1];
        }

        private void kodeDokter1Txt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                Modul_Data_Dokter_PopUp dokter = new Modul_Data_Dokter_PopUp(this);
        ...
        private void itemPenjualanDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (((e.ColumnIndex == 5) || (e.ColumnIndex == 4))&&(e.RowIndex >=0)&&(e.ColumnIndex >=0))
            {
                ...
                {
                    if ((itemPenjualanDataGridView.Rows[itemPenjualanDataGridView.CurrentCell.RowIndex].Cells[0].Value.ToString().ToLower().Contains("tuslah"))&&(itemPenjualanDataGridView.CurrentCell.ColumnIndex==4))
                    {
        ...
        {
            Modul_Ganti_Operator_Laporan gantiOperator = new Modul_Ganti_Operator_Laporan(null,this,this.login);
            gantiOperator.Show();
        }
        ...
        private void tambahTuslah()
        {
            if (!cariTuslah())
            {
                string[] strTuslah = new string[] { "TUSLAH", "TUSLAH","", "6000", "7000", "8000","1000"};
                setItem(strTuslah);
            }
            else
            {
                MessageBox.Show("Tuslah sudah diinput.");
            }
        }

        private void Modul_POS_Resep_Dokter_FormClosed(object sender, FormClosedEventArgs e)
        {
            login.Show();
        }

        private void hargaJualTxt_Click(object sender, EventArgs e)
        {
        }
        ...
        private void namaPasienTxt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                umurPasienTxt.Focus();
        ...
        private void umurPasienTxt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                alamatPasienTxt.Focus();
            }
        }

        private void alamatPasienTxt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                noResepTxt.Focus();
        ...
        private void noResepTxt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                kodeDokter1Txt.Focus();
            }
        }
